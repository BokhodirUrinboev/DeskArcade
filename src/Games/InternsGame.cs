...
/// is the highest level cleared. Only the interns, the hatch and the toolbar take the mouse.
/// </summary>
public sealed class InternsGame : MiniGame
{
    ...
    /// or just above the floor, the exit on the floor across the screen, and manholes in between.
    /// </summary>
    void NewLevel()
    {
        var a = Host.Arena;
    ...
    void EndLevel()
    {
        ...
        if (_passed)
        {
            ...
            Host.Fx.Popup(at, w.Saved == w.Total ? L.T("EVERYONE MADE IT!") : L.F("LEVEL {0} CLEARED", _level), Gold, 38, 2.8,
                L.F("{0} of {1} saved · {2} needed", w.Saved, w.Total, w.Needed));
            Host.Fx.Burst(at, Themes.Current.Confetti, 40, 520, 700, 7, 1.0);
            Host.Sound.Play("best", 0.8);
        }
        else
        {
            Host.Fx.Popup(at, L.T("NOT ENOUGH"), Colors.White, 36, 2.8, L.F("{0} of {1} saved · {2} needed", w.Saved, w.Total, w.Needed));
    ...
    void DrawScenery()
    {
        ...
        // the exit: a door with a green sign above it
        ...
        _scenery.Children.Add(Art.At(new Rectangle { Width = DoorW - 6, Height = DoorH - 4, Fill = Art.Brush(Color.FromRgb(255, 226, 150)), Opacity = 0.85 }, ex + 3, ey + 4));
        var sign = new Border
        {
    ...
    void DrawHatch(bool open)
    {
        ...
        _hatch.Children.Add(Art.At(new Rectangle { Width = HatchW, Height = HatchH, RadiusX = 3, RadiusY = 3, Fill = Art.Brush("#3A3F4E"), Stroke = Art.Brush("#1B1B22"), StrokeThickness = 1.5 }, x, y));
        if (open)
        {
        ...
        {
            _hatch.Children.Add(Art.At(new Rectangle { Width = HatchW - 8, Height = 3, Fill = Art.Brush("#8A90A2") }, x + 4, y + HatchH - 3));
            if (_phase != Phase.Playing)
            {
    ...
    void DrawPeople(InternsWorld w)
    {
        foreach (var it in w.Interns)
        {
            ...
            look.Umbrella.IsVisible = it.Umbrella && it.State == State.Falling;
    ...
    Look NewLook()
    {
        ...
        return new Look { Holder = holder, Flip = flip, Move = move, LegsA = legsA, LegsB = legsB, Arms = arms, Umbrella = umbrella, Brick = brick, Ring = ring };
    }

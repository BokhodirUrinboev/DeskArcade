...
public sealed class HockeyTable
{
    ...
    double _acc, _stuckT, _aimCpu, _aimMe, _cpuBackOff;
    bool _puckOnCpuSide;
    ...
    /// <summary>The computer's level: its mallet gets faster with every match it loses.</summary>
    ...
    public double CpuSpeed => Math.Min(1350, 620 + 110 * Level);
    ...
    {
        Puck = new Vec2(Arena.Center.X + side * Arena.Width * 0.22, Arena.Center.Y);
        PuckVel = default;
        _stuckT = 0;
    }
    ...
    public Vec2 CpuMove(double dt, bool holdHome)
    {
        ...
        var target = holdHome || _cpuBackOff > 0 ? Home(true) : AiTarget(Cpu, true);
        return MoveToward(Cpu, ClampSide(target, true), CpuSpeed * dt);
    }

    /// <summary>The same AI for the left mallet (demo mode).</summary>
    public Vec2 DemoMove(double dt) => MoveToward(Me, ClampSide(AiTarget(Me, false), false), CpuSpeed * dt);
    ...
    Vec2 AiTarget(Vec2 mallet, bool cpu)
    {
        ...
        if (!puckOnMySide || !PuckInPlay) return home;
        ...
        Vec2 behind = Puck - dir * (PuckR + MalletR - 4);

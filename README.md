...
- **39 games, 106 achievements**, a daily challenge, play-time stats, and English, Uzbek and Russian text.
...
- **Thirteen themes** (including a seasonal one) that dress the scoreboard, boards, tables, pieces and pet, with light
  ...
  visit each other over the LAN and, if you like, keep you company in the other games.
...
pet, CPU difficulty, theme, sound (and the pet's volume), accessibility, language, play over LAN, stats, shortcuts, updates, hide, exit. It is the way in on desktops
...
(and the pet's favourite nap spot), and all 106 achievements with their progress. Stats live in
...
**Tray → Pet → Pet keeps me company in games** (also in the ☰ menu, off by default) brings your pet along to the
other games: it sits on the taskbar (on the pier in Fishing), reacts to the game sounds, and joins in Hoops, Pong,
...
| `src/Net` | `LanLink`: pairing and messages between two copies on the local network; `DuelChannel`: reliable, ordered events for turn-based duels; `PetMail`: gifts for a co-worker's pet (format, receipts, rate limit, queue); `OfficeBoard`: the opt-in leaderboard; `RoomLink`: rooms of up to four players by code, for Durak and Last Card |
